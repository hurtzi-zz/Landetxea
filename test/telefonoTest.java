...
public class telefonoTest extends TestCase {
    ...
    public void testCreateGui2() throws InterruptedException {
        try {
            facade.setDataAccess(new DataAccessLocal());
            boolean buelta = facade.createOwner(123, "202149955487", "Owner", "Altuna", "owner", "000");
            facade.close();
            assertEquals(false, buelta);
        } catch (RemoteException e) {
            e.printStackTrace();
    ...
    public void testCreateGui3() throws InterruptedException {
        try {
            facade.setDataAccess(new DataAccessLocal());
            boolean buelta = facade.createOwner(1234567891, "202149955487", "Owner", "Altuna", "owner", "000");
            facade.close();
            assertEquals(false, buelta);
        } catch (RemoteException e) {
            e.printStackTrace();
    ...
    public void testCreateGui5() throws InterruptedException {
        try {
            facade.setDataAccess(new DataAccessLocal());
            boolean buelta = facade.createOwner(-987654321, "202149955487", "Owner", "Altuna", "owner", "000");
            facade.close();
            assertEquals(false, buelta);
        } catch (RemoteException e) {
            e.printStackTrace();
